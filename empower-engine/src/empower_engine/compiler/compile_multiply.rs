...
pub fn execute_debug_multiply_node(node_to_compile_key: EmpowerKey, nodes: &mut HashMap<EmpowerKey, Node>, input_ports: &mut HashMap<EmpowerKey, InputPort>, output_ports: &mut HashMap<EmpowerKey, OutputPort>)
{
    let node_to_compile = nodes.get_mut(&node_to_compile_key).unwrap(); // @TODO, handle exception

    if node_to_compile.node_type != NodeType::Multiply
    {
    ...
    print!("multiply node ({}) : [{}] [{}], [{}]", node_to_compile_key, input_port_1.value, input_port_2.value, output_port.value);
}
