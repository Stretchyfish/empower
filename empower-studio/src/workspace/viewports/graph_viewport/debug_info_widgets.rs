...
pub fn nodes_debug_info_show(ui: &mut egui::Ui, graph_editor: &GraphEditor)
{
    ...
    for (node_index, node_key) in node_execution_order_keys.iter().enumerate()
    {
        ...
        let display_node = graph_editor.display_nodes.get(&node_key).expect("Tried to access node in debug info which was not available");
        ...
        {
            x: display_node.position.x + display_node.size.x - number_box_lengths - number_box_horizontal_offset,
            y: display_node.position.y
        };
        let number_rect_bottom_right_corner = egui::Pos2
        {
            x: display_node.position.x + display_node.size.x - number_box_horizontal_offset,
